package com.mogoroom.hadoop.base.dao;
...
public class CommonDAO {
     ...
     * @return 返回实体类{@code<T>}
     */
    public <T> T getEntity(String strSql, Map<String, Object> paras, Class<T> entityName) {
        Query query = this.getEntityManager().createNativeQuery(strSql, entityName);
     ...
     * @return 返回实体类{@code<T>}
     */
    public <T> List<T> getEntityList(String strSql, Map<String, Object> paras, Class<T> entityName) {
        Query query = this.getEntityManager().createNativeQuery(strSql, entityName);
        for (String key : paras.keySet()) {
            query.setParameter(key, paras.get(key));
        }
     ...
     */
    public <T> PageData<T> getEntityListPage(String strSql, Map<String, Object> paras, Class<T> entityName, int start, int limit) {
        Query query = this.getEntityManager().createNativeQuery(strSql, entityName);
        for (String key : paras.keySet()) {
            query.setParameter(key, paras.get(key));
        }
